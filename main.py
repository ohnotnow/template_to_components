...
def convert_template(filename, a11y=False, responsive=False, css=False, model="gpt-4o", component=True):
    ...
    with open(filename, "r") as file:
        ...
        if component:
            messages = [
                {"role": "system", "content": system_prompts["tailwind_to_component"]},
                {"role": "user", "content": f"Could you convert this laravel blade template into a blade component?  Please respond with only the converted code - no additional text.  This will help me copy and paste it directly into my work.\n\n```{updated_template}```"},
            ]
            response = talk_to_llm(messages, model=model)
            updated_template = response.message
            with open(f"components/{os.path.basename(filename)}", "w") as file:
